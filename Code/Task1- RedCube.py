    ...
    for t in time_range:  # Generate a sequence of images as a function of time
        ...
        for i in range(r):
            for j in range(c):
                ...
                #p1_homo = np.dot(K, p1)  # Apply the camera projection matrix
                #p1_homo /= p1_homo[2]    # Normalize the homogeneous coordinates
                #set ir to the x-value of this point
                # set jr to the y-value of this point
                #ir, jr, _ = p1_homo.astype(int)  # Cast to int to index the array A
